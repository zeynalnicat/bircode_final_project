...
class SignUpViewModel @Inject constructor(private val signUpUseCase: SignUpUseCase): ViewModel(),
    ...
    private fun onSubmit(){
        ...
        if(password.length<6){
            _state.update { it.copy(passwordError = AppStrings.passwordLenghtError, loading = false) }
        }
        ...
        if(emailError.isEmpty() && passwordError.isEmpty() && nameError.isEmpty()){
            ...
            viewModelScope.launch {
                when(val res = signUpUseCase.invoke(email,name,password)){
                    is Result.Error -> _effect.emit(SignUpUiEffect.OnShowError(res.message))
                    is Result.Success<*> ->  navController?.navigate(ScreenModel.Home.route,){popUpTo(ScreenModel.SignUp.route) { inclusive = true }}
                }
            }
